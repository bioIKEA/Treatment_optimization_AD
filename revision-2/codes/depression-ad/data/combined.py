...
medication_file['CMMED'] = np.where((medication_file['CMREASON'] == "depression"), 'depression', medication_file['CMMED'])
...
def check_group(group):
    if isinstance(group['CMREASON'], pd.Series) or isinstance(group['CMMED'], pd.Series):
        for row_id, row in group.iterrows():
            if set(['depression','ad']).intersection(set(row['CMREASON'])) or set(['depression','inhibitors','namenda']).intersection(set(row['CMMED'])): 
                return group
