...
def QLearning(q_table, train_data, alpha):
...
weighted_group=np.zeros((100,9),dtype=np.float32)
arr=[]
for repetition in range(100):
    ...
    best_q_policy_list=[0]*9
    old_q_reward_list=[-100]*9
    ...
    for internal_repitition in range(50):
        ...
        for alpha in alphas: 
            ...
            q_tables=QLearning(q, full_train_set, alpha)
            q_policy=derive_policy(q_tables, epsilon)
            q_policy_list.append(q_policy)

            #######weighted_list########## 

            arr.append(repetition)
        for count_policy in range(9): 
            current_q_reward=weighted_importance_sampling(q_policy_list[count_policy], val_set,behavior_policy)
            if current_q_reward>=old_q_reward_list[count_policy]:
                best_q_policy_list[count_policy]=q_policy_list[count_policy]
                old_q_reward_list[count_policy]=current_q_reward
    ...
    weighted_group[repetition,:]=[weighted_importance_sampling(best_q_policy,test_set,behavior_policy) for best_q_policy in best_q_policy_list]
...
main=ax.boxplot(weighted_group, patch_artist=True,  positions=[1,2.5,4,5.5,7,8.5,10,11.5,13])
m = weighted_group.mean(axis=0)
st = weighted_group.std(axis=0)
ax.set_title('Ad only')
for i, line in enumerate(main['medians']):
    x, y = line.get_xydata()[1]
    text = ' μ={:.2f}\n σ={:.2f}'.format(m[i], st[i])
    ax.annotate(text, xy=(x, y))
ax.set_xticks([1,2.5,4,5.5,7,8.5,10,11.5,13])
ax.set_xticklabels(["a=0.1","a=0.2","a=0.3" ,"a=0.4","a=0.5","a=0.6","a=0.7","a=0.8","a=0.9"], rotation=20)
ax.set_ylim(-10,5)
colors=["forestgreen", "blue", "dimgray", "lightgreen", "slategray", "darkmagenta", "green", "tan", "tomato"]
for patch, color in zip(main['boxes'],colors):
    patch.set_facecolor(color)
# plt.xticks([1, 2,3,4,5], ["policy_iteration","q", "zero", "random",'copy'], rotation=10)
# plt.ylim(-10, 5)
 

plt.savefig('onlyalpha0-9v4.png')
plt.show()
