...
merged_file= pd.read_csv("/Users/kritibbhattarai/Desktop/internship/Alzheimer's/python/new-w-bootstrap/hyper-ad-dep/data/merged_wo-sa.csv")
...
for index,row in merged_file.iterrows():
    if ( row['ADAS13_pre'] <= 25.164999961853027 ) :
        ...
        if ( row['ADAS13_pre'] <= 34.834999084472656 ) :
            if ( row['CDRSB_pre'] <= 3.25 ) :
                state=5
            else :
                if ( row['ADAS13_pre'] <= 30.835000038146973 ) :
                    state=6
...
for med_list in merged_data['CMMED']:
    ...
    if set(['inhibitors', 'namenda']).issubset(set(med_list)):
        ...
        action=0
    # elif set(['depression']).issubset(set(med_list)):
    #     action=5
    else:
        action=5
...
merged_data25.to_csv('merged_final_9_states.csv', index = False)  # Export merged pandas DataFrame
