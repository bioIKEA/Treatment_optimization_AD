...
data_path='/Users/kritibbhattarai/Desktop/Treatment_optimization_AD/adni+aibl/algorithm/ad-hyp-dep/data/adni-data/data-processing/'


merged_file= pd.read_csv("processed_adni_ad_dep_hyp_lab_file.csv")
...
for index,row in merged_file.iterrows():
    if ( row['CDGLOBAL_pre'] <= 0.75 ) :
        if ( row['LIMMTOTAL_pre'] <= 9.865745067596436 ) :
            if ( row['CDGLOBAL_pre'] <= 0.21301774680614471 ) :
                state=0
            else :
                if ( row['LDELTOTAL_pre'] <= 1.5 ) :
                    state=1
                
                else :
                    state=2
        else :
            if ( row['LDELTOTAL_pre'] <= 6.5 ) :
                state=3
            else :
                if ( row['LIMMTOTAL_pre'] <= 12.5 ) :
                    state=4
                else :
                    state=5
    else :
...
merged_data25.to_csv('adni_ad_dep_hyp_lab_w_states.csv', index = False)  # Export merged pandas DataFrame
