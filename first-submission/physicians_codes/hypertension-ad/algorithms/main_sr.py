...
data_path="/Users/kritibbhattarai/Desktop/Treatment_optimization_AD/physicians_codes/hypertension-ad/data/sr/"

data1= pd.read_csv(data_path+'sr_final.csv', low_memory=False)
...
for repetition in range(100):
    ...
    weighted_group[repetition,:]=[weighted_importance_sampling(best_q_policy,test_set,behavior_policy),weighted_importance_sampling(best_copy_policy,test_set,behavior_policy)]
...
###################################################################################plotting################################################################
print("sr_physician= ",weighted_group)
print("sr_list= ",weighted_group.tolist())
...
ax.set_title('SR Physician')
for i, line in enumerate(main['medians']):
    x, y = line.get_xydata()[1]
    text = ' μ={:.2f}\n σ={:.2f}'.format(m[i], st[i])
    ax.annotate(text, xy=(x, y))
ax.set_xticks([1, 2])
ax.set_xticklabels(["q_learning", 'clinician'], rotation=10)
ax.set_ylim(-20,10)

colors=["forestgreen",   "slategray"]
 
for patch, color in zip(main['boxes'],colors):
    patch.set_facecolor(color)

plt.savefig('SR_phy.png')
# show plot
plt.show()
