...
merged_file= pd.read_csv("/Users/kritibbhattarai/Desktop/Treatment_optimization_AD/physicians_codes/depression-ad/data/jr/jr_phy_wo_sa.csv")
...
for index,row in merged_file.iterrows():
    if ( row['ADAS13_pre'] <= 28.5 ) :
        if ( row['ADAS13_pre'] <= 22.835000038146973 ) :
            state=0
            ...
            if ( row['FDG_pre'] <= 5.766028165817261 ) :
                state=1
        ...
        if ( row['ADAS13_pre'] <= 36.5 ) :
            if ( row['RAVLT.immediate_pre'] <= 15.5 ) :
                state=3
                
            
            else :
                if ( row['CDRSB_pre'] <= 6.25 ) :
                    state=4
...
merged_data25.to_csv('jr_final.csv', index = False)  # Export merged pandas DataFrame
