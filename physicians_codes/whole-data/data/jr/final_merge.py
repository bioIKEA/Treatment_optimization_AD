...
merged_file= pd.read_csv("/Users/kritibbhattarai/Desktop/Treatment_optimization_AD/physicians_codes/whole-data/data/jr/jr_phy_wo_sa.csv")
...
for index,row in merged_file.iterrows():
    if ( row['ADAS13_pre'] <= 29.5 ) :
        if ( row['CDRSB_pre'] <= 3.25 ) :
            if ( row['RAVLT.immediate_pre'] <= 27.5 ) :
                if ( row['ADAS13_pre'] <= 23.164999961853027 ) :
                    state=0
            ...
            if ( row['FDG_pre'] <= 6.025132417678833 ) :
                if ( row['RAVLT.immediate_pre'] <= 24.5 ) :
                    state=4
