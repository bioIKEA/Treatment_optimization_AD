...
merged_file= pd.read_csv("/Users/kritibbhattarai/Desktop/Treatment_optimization_AD/physicians_codes/hyper-ad-dep/data/jr/jr_phy_wo_sa.csv")
...
for index,row in merged_file.iterrows():
    if ( row['ADAS13_pre'] <= 28.5 ) :
        if ( row['CDRSB_pre'] <= 3.75 ) :
            ...
            if ( row['RAVLT.immediate_pre'] <= 23.5 ) :
                state=2
        ...
        if ( row['ADAS13_pre'] <= 36.5 ) :
            if ( row['FDG_pre'] <= 5.051861763000488 ) :
                state=4
