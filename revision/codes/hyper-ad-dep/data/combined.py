...
def check_group(group):
    ...
    if isinstance(group['CMREASON'], pd.Series) or isinstance(group['CMMED'], pd.Series):
        for row_id, row in group.iterrows():
            # if ('ad' in row['CMREASON'] or 'inhibitors' in row['CMMED'] or 'namenda' in row['CMMED']) and (('depression' in row['CMREASON'] or 'depression' in row['CMMED']) or ('hypertension' in row['CMREASON'] or 'hypertension' in row['CMMED'])): 
            #     return group
            ...
            if ('hypertension' in row['CMREASON'] or 'hypertension' in row['CMMED']): 
                hyp_checker=True
        if ad_checker and dep_checker and hyp_checker:
            return group
ad_dep_hyp_data= whole_data_without_states.groupby('RID').apply(check_group)
print(ad_dep_hyp_data.head(5))
